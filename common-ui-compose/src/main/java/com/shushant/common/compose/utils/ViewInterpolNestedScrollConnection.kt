...
/**
 * This is a workaround for providing scroll behaviour for ComposeViews, which means that nested scrolling layouts
 ...
 * - Using a `ComposeView` + `LazyColumn()` in a `MotionLayout` with 'Toolbar'.
 *
 ...
 *         modifier = Modifier.nestedScroll(rememberViewInterpolNestedScrollConnection())
 *     ) {
 *         LazyColumn() {
 *             // composableContent()
 *         }
...
class ViewInterpolNestedScrollConnection(
