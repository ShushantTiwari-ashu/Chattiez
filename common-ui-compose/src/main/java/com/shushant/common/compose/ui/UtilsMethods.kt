...
object UtilsMethods {
    ...
    fun validationLogin(email: String, password: String): ValidationResult {
        ...
        if (password.isBlank() || password.isEmpty()) return ValidationResult(
            false,
    ...
    fun validationRegister(email: String, password: String, confirm: String): ValidationResult {
        ...
        if (password.isBlank() || password.isEmpty() && confirm.isBlank() || confirm.isEmpty()) return ValidationResult(
            false,
            "Invalid login password"
        )

        if (password != confirm) return ValidationResult(
            false,
