    ...
    Box(modifier = Modifier) {
        Column(
            ...
            WheelDatePicker(
                ...
                startDate = if (userState.dob.isNotEmpty()) LocalDate.parse(userState.dob) else LocalDate.now(),
                modifier = Modifier
                    .height(200.dp)
                    .fillMaxWidth()
                    .padding(horizontal = 20.dp),
            ...
            ) { snappedDate -> dob.invoke(snappedDate.toString()) }

        }

        ChattiezButton(
            buttonText = stringResource(R.string.continue_text),
            enabled = userState.dob.isNotEmpty(),
            modifier = Modifier
